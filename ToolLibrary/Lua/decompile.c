...
StringBuffer* PrintLogicExp(StringBuffer* str, int dest, LogicExp* exp, int inv_, int rev) {
	...
	StringBuffer* ret = PrintLogicItem(str, exp, inv_, rev);
	return ret;
}
...
char* PrintFunction(Function* F) {
	...
	if (IsMain(F->f)) {
		...
		if (F->funcBlock->code[0] == '.' && F->funcBlock->code[1] == '.' && F->funcBlock->code[2] == '.' && F->funcBlock->code[3] == 0) {
			PrintAstSub(F->funcBlock, buff, 0);
		}
		else {
