...
void DoApp(std::vector<std::string>& args) {
	...
	if (m == mode::C_ARCH) {
		...
		for (auto it = arch.mResources.begin(); it != arch.mResources.end(); it++) {
			result = arch.GetResourceName(it->mNameCRC);
		...
		if (!T3EffectCache_Legacy::LoadPackage(package, effectPackage)) {
		...
		for (int i = 0; i < package.mHeader.mProgramCount; i++) {
			...
			for (int i = 0; i < decltype(pProgram->mEffectFeatures)::EnumValCount; i++) {
				if (pProgram->mEffectFeatures[(T3EffectFeature)i])
					writer.mWriter.String(T3EffectCache_Legacy::GetFeatureName((T3EffectFeature)i));
			}
		...
		for (int i = 0; i < package.mHeader.mShaderCount; i++) {
			...
			writer.mWriter.String(T3EffectCache_Legacy::GetShaderTypeName(pShader->mShaderType));
			...
			for (int i = 0; i < decltype(pShader->mNeededParameters)::EnumValCount; i++) {
				if (pShader->mNeededParameters[(T3EffectParameterType)i])
					writer.mWriter.String(T3EffectCache_Legacy::GetParameterTypeName((T3EffectParameterType)i));
			}
			writer.mWriter.EndArray();
			T3EffectCache_Legacy::LoadShaderPayload(package, *pShader, shaderBuffer, effectPackage);
